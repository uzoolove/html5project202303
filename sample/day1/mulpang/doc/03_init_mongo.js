...
function registMember(){
	...
	db.member.insertMany(members, registShop);
...
function registShop(){
	...
	db.shop.insertMany(shops, registCoupon);
...
function registCoupon(){
    ...
    // 4. 쿠폰 데이터 등록 완료 시 후기 데이터 등록
		db.coupon.insertMany(coupons, registEpilogue);
...
function registEpilogue(){
	...
	// 5. 후기 데이터 등록 완료 시 구매 데이터 등록
	db.epilogue.insertMany(epilogues, registPurchase);
...
function registPurchase(){
	var purchases = [
		...
		},
		{
			...
			epilogueId: epilogueId[6],
 			regDate: getTime(-4, 40000)
		}
	];
	
	// 6. 구매 데이터 등록 완료 시 등록한 쿠폰 목록을 확인한다.
	db.purchase.insertMany(purchases, findCoupon);
...
// 모든 쿠폰명을 출력한다.
function findCoupon(){
	db.coupon.find({}, {projection: {_id: 0, couponName: 1}}).toArray(function(err, result){
		console.info(result);
		console.info("6. 쿠폰 " + result.length + "건 조회됨.");
		client.close();
	});
}
